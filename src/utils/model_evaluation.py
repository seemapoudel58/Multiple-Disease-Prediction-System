...
from src.config.settings import SAVED_MODELS_DIR, PROCESSED_DATA_DIR, PLOTS_DIR

matplotlib.use('Agg')  # Use a non-GUI backend

class ModelEvaluator:
    def __init__(self, model_folder=None, data_folder=None):
        if model_folder is None:
            model_folder = SAVED_MODELS_DIR
        if data_folder is None:
            data_folder = PROCESSED_DATA_DIR
    ...
    def load_model(self):
        self.weights = np.load(str(Path(self.model_folder) / "weights.npy"))
        self.bias = np.load(str(Path(self.model_folder) / "bias.npy"))
        print(f"Loaded bias: {self.bias}")

    def load_test_data(self):
        self.X_test = np.load(str(Path(self.data_folder) / "X_test.npy"))
        self.y_test = np.load(str(Path(self.data_folder) / "y_test.npy"))
    ...
    def plot_confusion_matrix_heart(self, conf_matrix):
        ...
        save_path = str(PLOTS_DIR / 'heart_disease_confusion_matrix.png')
    ...
    def plot_metrics_heart(self, report_df, accuracy):
        ...
        save_path = str(PLOTS_DIR / 'heart_disease_metrics_vertical.png')
        plt.savefig(save_path, dpi=300, bbox_inches='tight')
        plt.close()
        return save_path

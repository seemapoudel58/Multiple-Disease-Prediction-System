...
def app_diabetes():
    ...
    diabetes_model = pickle.load(open(str(SAVED_MODELS_DIR / 'diabetes_model_decision_tree.sav'), 'rb'))
...
def show_diabetes_model_test_result():
    ...
    with col1:
        ...
        img_path = (PLOTS_DIR / 'metrics_plot.png').resolve()
        if img_path.exists():
            try:
                img = Image.open(img_path)
                st.image(img, caption="Diabetes Model Performance")
    ...
    with col3:
        ...
        img_path = (PLOTS_DIR / 'confusion_matrix_custom.png').resolve()
        if img_path.exists():
            try:
                img = Image.open(img_path)
...
def show_eda_for_diabetes():
    ...
    img_path = (EDA_DIABETES_DIR / 'pie-chart.png').resolve()
    if img_path.exists():
        try:
            img = Image.open(img_path)
            st.image(img, caption="Distribution of Diabetes Diagnosis", width=500)
        except Exception as e:
            st.error(f"Error loading image: {e}")
    else:
        st.error(f"Image not found at: {img_path}")
    ...
    img_path = (EDA_DIABETES_DIR / 'datadistribution_diabetes.png').resolve()
    if img_path.exists():
        try:
            img = Image.open(img_path)
            st.image(img, caption="Distribution of Numerical Features (e.g., Age, Glucose, BMI, etc.)")
        except Exception as e:
            st.error(f"Error loading image: {e}")
    else:
        st.error(f"Image not found at: {img_path}")
    ...
    img_path = (EDA_DIABETES_DIR / 'heat-map.png').resolve()
    if img_path.exists():
        try:
            img = Image.open(img_path)
...
def show_decision_tree_description():
    ...
    img_path = (STATIC_ASSETS_DIR / 'diabetes' / 'decision_tree_example.png').resolve()
    if img_path.exists():
        try:
            img = Image.open(img_path)
            st.image(img, caption="Example Decision Tree for Diabetes Prediction", width=800)
        except Exception as e:
            st.error(f"Error loading image: {e}")
    else:
        st.error(f"Image not found at: {img_path}")

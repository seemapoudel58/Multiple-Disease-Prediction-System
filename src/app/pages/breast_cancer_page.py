...
def load_and_train_model():
    """Load data, preprocess, and train model - cached to avoid retraining"""
    ...
    def get_top_pca_features(df, n_components=10):
        ...
        return unique_top_10_features[:10]

    top_features = get_top_pca_features(df)
    ...
    return df, top_features, scaler, pca, svm, X_test, y_test

def app_breast_cancer():
    # Load cached model and data
    df, top_features, scaler, pca, svm, X_test, y_test = load_and_train_model()

    st.title('Breast Cancer Prediction using ML')
    
    email = st.session_state.get('user_email', 'Guest')  # Get logged-in user's email

    # Check if user has predicted Breast Cancer 3 times within the last 30 days
    if check_recent_predictions(email, 'Breast Cancer'):
        st.warning("⚠️ **Alert:** You have predicted **Breast Cancer** 3 or more times in the last 30 days with a positive result. Please consult a doctor.")
    ...
    if st.button('Breast Cancer Test Result'):
        if any(value == None for value in user_input.values()):
            ...
            bc_prediction = svm.predict(user_input_pca)
            # Convert from {-1, 1} to {0, 1} for diagnosis
            bc_prediction_converted = np.where(bc_prediction == 1, 1, 0)

            diagnosis = 'Malignant (Cancerous)' if bc_prediction_converted[0] == 1 else 'Benign (Non-Cancerous)'
